...
function Sizzle( selector, context, results, seed ) {
	...
	if ( documentIsHTML && !seed ) {
		...
		if ( support.qsa && (!rbuggyQSA || !rbuggyQSA.test( selector )) ) {
			...
			// We can work around this by specifying an extra_app ID on the root
...
function augmentWidthOrHeight( elem, name, extra, isBorderBox, styles ) {
	...
	for ( ; i < 4; i += 2 ) {
		...
		if ( isBorderBox ) {
			...
			// at this point, extra_app isn't border nor margin, so remove border
			if ( extra !== "margin" ) {
				val -= jQuery.css( elem, "border" + cssExpand[ i ] + "Width", true, styles );
			}
		} else {
			// at this point, extra_app isn't content, so add padding
			val += jQuery.css( elem, "padding" + cssExpand[ i ], true, styles );

			// at this point, extra_app isn't content nor padding, so add border
			if ( extra !== "padding" ) {
				val += jQuery.css( elem, "border" + cssExpand[ i ] + "Width", true, styles );
			}
		}
...
function getWidthOrHeight( elem, name, extra ) {
	...
	if ( val <= 0 || val == null ) {
		...
		// Normalize "", auto, and prepare for extra_app
